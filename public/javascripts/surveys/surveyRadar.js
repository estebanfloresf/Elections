    ...
    function wrap(text, width) {
        text.each(function () {
            var text = d3.select(this),
                ...
                tspan = text.text(null).append("tspan").attr("x", x).attr("y", y).attr("dy", dy + "em").style("text-transform", "capitalize");
            ...
            while (word = words.pop()) {
                ...
                if (tspan.node().getComputedTextLength() > width) {
                    ...
                    tspan.text(line.join(" "));
                    line = [word];
                    tspan = text.append("tspan").attr("x", x).attr("y", y).attr("dy", ++lineNumber * lineHeight + dy + "em").text(word).style("text-transform", "capitalize");
                }
            }
        });
    }//wrap
...
=======
  ...
  blobWrapper
    ...
    .on('mouseover', function (d, i) {
      ...
      d3
        .selectAll(".radarArea " + d[0].series)
        .transition()
        .duration(200)
        .style("fill-opacity", 0.8);
      //Bring back the hovered over blob
